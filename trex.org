...
** Periodic boundary calculations (pbc group)
  ...
  | ~k_point~        | ~float~ | ~[pbc.k_point_num,3]~ | ~(3,pbc.k_point_num)~   | $k$-point sampling                                   |
...
** Basis set (basis group)
  ...
  We consider here basis functions centered on nuclei. Despite this 
  restriction, one can introduce *dummy atoms* (artificial centers
  ...
  $s$ is centered on a point $\mathbf{R}_A$, it has a given
  angular momentum $\ell_s$, and is defined by a radial function
...
*** Gaussian-type and Slater-type orbitals
  ...
  Contracted Gaussian basis sets, such as those available from the Basis
  Set Exchange, are typically expressed in terms of normalized
  /primitive/ functions.
  ...
  This is the normalization of the /full primitive basis function/
  (radial and angular parts in the chosen TREXIO convention), not only
  of the radial Gaussian factor.  Normalization is applied
  independently for each primitive function. If this normalization is
  not required, then $f_{ks} = 1$.
  ...
  The ~shell_factor~ stores this shell-level factor $\mathcal{N}_s$.

*** Flexibility and validation

  The normalization factors $f_{ks}$, $\mathcal{N}_s$, and $\mathcal{N}_i'$
  described above are designed to make TREXIO flexible enough to be
  compatible with most quantum chemistry codes.  Their sole purpose is
  to provide additional degrees of freedom in the expression of the
  basis functions, and they do not have a single rigid definition.
  Because of this built-in redundancy, the same physical basis function
  can be stored in multiple equivalent ways.

  The only requirement is *self-consistency*: when a basis function is
  evaluated numerically at a point in space, the result must be the
  same function that was used to compute the integrals stored in the
  file.  Concretely, this means that the product
  \[
  \mathcal{N}_s \, f_{ks} \, a_{ks}
  \]
  must remain invariant under any redistribution of the normalization
...
*** Numerical orbitals
    ...
    Communications 2009]. The radial function $R_s$ is of the form
    \[
...
information about auxiliary basis functions. See basis group
documentation.
...
** Atomic orbitals (ao group)
   ...
   $\mathcal{N}_s$ in the ~basis~ group is appropriate for instance
   ...
   ~ao~ group is the ratio $\frac{\mathcal{N}_{xy}}{\mathcal{N}_{z^2}}$.
...
*** One-electron integrals (~ao_1e_int~ group)
    ...
    - $\hat{\mu}_x = -x$, $\hat{\mu}_y = -y$, $\hat{\mu}_z = -z$ : dipole moment operators
  ...
  | ~dipole_x~            | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_x \vert q \rangle$                            |
  | ~dipole_y~            | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_y \vert q \rangle$                            |
  | ~dipole_z~            | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_z \vert q \rangle$                            |
  ...
  | ~dipole_x_im~         | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_x \vert q \rangle$ (imaginary part)           |
  | ~dipole_y_im~         | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_y \vert q \rangle$ (imaginary part)           |
  | ~dipole_z_im~         | ~float~ | ~[ao.num, ao.num]~   | ~(ao.num, ao.num)~      | $\langle p \vert \hat{\mu}_z \vert q \rangle$ (imaginary part)           |
...
*** One-electron integrals (~mo_1e_int~ group)
  ...
  | ~dipole_x~            | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_x \vert j \rangle$                            |
  | ~dipole_y~            | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_y \vert j \rangle$                            |
  | ~dipole_z~            | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_z \vert j \rangle$                            |
  ...
  | ~dipole_x_im~         | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_x \vert j \rangle$ (imaginary part)           |
  | ~dipole_y_im~         | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_y \vert j \rangle$ (imaginary part)           |
  | ~dipole_z_im~         | ~float~ | ~[mo.num, mo.num]~   | ~(mo.num, mo.num)~      | $\langle i \vert \hat{\mu}_z \vert j \rangle$ (imaginary part)           |
...
** Auxiliary atomic orbitals (~auxao~ group)

  Duplicate of the atomic orbitals group see documentation above.
  Used with the ~auxbasis~ group to define auxiliary basis functions.

  #+CALL: json(data=ao, title="auxao", replace_group="ao")
  ...
  #+begin_src python :tangle trex.json
      "auxao": {
	      "cartesian" : [ "int"  , []           ]
	,           "num" : [ "dim"  , []           ]
	,         "shell" : [ "index", [ "auxao.num" ] ]
	, "normalization" : [ "float", [ "auxao.num" ] ]
      } ,
  #+end_src
...
** Density fitting coefficients (~densityfit~ group)

  The molecular electron density can be approximated by an expansion
  on an auxiliary basis such that:

  \[
  \rho(\mathbf{r}) \simeq \sum_{i=1}^{N} a_i f_i(\mathbf{r}).
  \]

  The $a_i$ are the $N$ (~auxao.num~) density fitting coefficients.
  The auxiliary basis is defined within the ~auxbasis~ and ~auxao~
  groups.

  Density-fitted operators are identified with an integer:

  | Integer | Operator                                        |
  |---------+-------------------------------------------------|
  |       0 | Overlap ($\delta(\mathbf{r}_1 - \mathbf{r}_2)$) |
  |       1 | Coulomb ($1/r_{12}$)                            |
...
*** Data definitions
  ...
  | Variable      | Type    | Row-major Dimensions          | Column-major Dimensions       | Description                                |
  |---------------+---------+-------------------------------+-------------------------------+--------------------------------------------|
  | ~num~         | ~dim~   |                               |                               | Number of density-fitted operators         |
  | ~coefficient~ | ~float~ | ~[densityfit.num, auxao.num]~ | ~(auxao.num, densityfit.num)~ | Density fitting coefficients               |
  | ~operator~    | ~int~   | ~[densityfit.num]~            | ~(densityfit.num)~            | Density-fitted operators, as defined above |


   #+CALL: json(data=densityfit, title="densityfit")
   ...
   #+begin_src python :tangle trex.json
       "densityfit": {
		   "num" : [ "dim"  , []                                ]
	 , "coefficient" : [ "float", [ "densityfit.num", "auxao.num" ] ]
	 ,    "operator" : [ "int"  , [ "densityfit.num" ]              ]
       } ,
   #+end_src
