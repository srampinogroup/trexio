...
#+SETUPFILE: docs/theme.setup
  ...
  library is written in C.
   ...
   #+begin_src json :tangle trex.json :exports none
{
   #+end_src
...
* Metadata (metadata group)
  ...
  **Note:** The ~unsafe~ attribute of the ~metadata~ group indicates
  ...
  #+RESULTS:
  :results:
  #+begin_src json :tangle trex.json
      "metadata": {
                 "code_num" : [ "dim", []                        ]
...
** Molecular orbitals (mo group)
  ...
  | ~electronic_temperature~ | ~float~ |             |                         | Electronic temperature in atomic units used to determine the occupations |
  ...
  | ~k_point~        | ~index~ | ~[mo.num]~           | ~(mo.num)~              | For periodic calculations, the $k$ point to which each MO belongs        |
   ...
   When the occupations are generated from a finite-temperature
   calculation, ~electronic_temperature~ stores the corresponding
   electronic temperature in atomic units so that the wave function can
   be restarted consistently.
...
print("""#+begin_src json :tangle trex.json""")
if title != "End":
    print("""    "%s": {"""%(title))
    indent = "        "
    f1 = 0 ; f2 = 0 ; f3 = 0
