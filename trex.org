...
** Density fitting coefficients (dfc group)

  The molecular electron density can be approximated by an expansion
  on an auxiliary basis such that:

  \[
  \rho(\mathbf{r}) \simeq \sum_{i=1}^{N_{\mathrm{abs}}} a_i f_i(\mathbf{r}).
  \]

  The $a_i$ are the density fitting coefficients. The auxiliary basis
  is defined within the ~auxbasis~ group.

   #+NAME: dfc
  | Variable      | Type    | Row-major Dimensions | Column-major Dimensions | Description                                   |
  |---------------+---------+----------------------+-------------------------+-----------------------------------------------|
  | ~type~        | ~str~   |                      |                         | Free text to identify the set of coefficients |
  | ~num~         | ~dim~   |                      |                         | Number of coefficients                        |
  | ~coefficient~ | ~float~ | ~[dfc.num]~          | ~(dfc.num)~             | Density fitting coefficients                  |


   #+CALL: json(data=dfc, title="dfc")
